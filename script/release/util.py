...
class ReleaseRepo:
    ...
    def local(self) -> "LocalRepo":
        path = Path(__file__).parent.parent.parent.parent / "release" / self.name
...
class LocalRepo:
    ...
    def _prepare_remotes(self, upstream: ReleaseRepo | None) -> None:
    ...
    def prepare(self, upstream: ReleaseRepo | None = None) -> None:
        # Clone
        if not self.path.exists():
            run("gh", "repo", "clone", self.rrepo.full_name, self.path)

        self._prepare_remotes(upstream)

        # Check worktree is ready
        self.git("diff", "--quiet")
        self.git("clean", "-dfx", silent=True)

        # Fetch recent changes
        self.git("fetch", "--all", "--prune", "--prune-tags", silent=True)
        if upstream:  # Some tags may have been pruned away
    ...
    def push(self, branch: str, upstream: bool = True) -> None:
        if upstream:
...
def find_pr(grepo: Repository, head: str, base: str) -> PullRequest | None:
...
def create_pr_url(
    ...
    url = f"{base.url}/compare/{base_branch}...{head.owner}:{head.name}:{head_branch}"
...
def _update_cmake_set(text: str, component: str, value: int) -> str:
    return re.sub(text, rf"set\({component}\s+\d+ ", f"set({component} {value} ")


def get_cmake_version(grepo: Repository, ref: str) -> tuple[Version, bool]:
    ...
    return Version(major, minor, patch), bool(is_release)


def set_cmake_version(lrepo: LocalRepo, version: Version, is_release: bool) -> None:
    cmakelists = lrepo.path / "src" / "CMakeLists.txt"
    text = cmakelists.read_text()
    text = _update_cmake_set(text, "LEAN_VERSION_MAJOR", version.major)
    text = _update_cmake_set(text, "LEAN_VERSION_MINOR", version.minor)
    text = _update_cmake_set(text, "LEAN_VERSION_PATCH", version.patch)
    text = _update_cmake_set(text, "LEAN_VERSION_IS_RELEASE", int(is_release))
